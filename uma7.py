#!/usr/bin/env python3
"""
UMA — Unknown Morphic Agent  v7.0
===================================
...
with cos(heading) so we turn-then-go instead of plowing through bends.
...
def run():
    ...
    print("UMA v7.0 — checking services...")
    ...
    print("UMA v7.0 running.  Ctrl-C to stop.\n")
    ...
    while True:
        ...
        if NAV_MODE == "vlm":
            ...
            if heading_c is not None:
                ...
                heading = heading_c * decay + bias
                speed   = SPEED_FLOOR + (1 - SPEED_FLOOR) * max(0.0, math.cos(math.radians(heading)))
                eb = f"  edge{bias:+.0f}" if bias else ""
                print(f"  🗺  heading {heading:+.1f}° (vlm {heading_c:+.0f}° conf={conf} "
                      f"age={age:.1f}s{eb})  →  spd {speed:.2f}")
                move_dir(heading, speed)
            else:
                print("  🗺  waiting for first VLM heading — forward")
                move_dir(bias, 1.0)
        else:
